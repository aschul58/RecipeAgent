...
from query_recipes import search_recipes
from enrichment import enrich_if_needed
from notion_api import get_recipes
from llm_answer import format_plan_with_llm
...
def tool_plan(pantry_query: str, top_k: int = 5, strict: bool = True) -> List[Dict[str, Any]]:
    """
    1) Keyword-Vorfilter (query_recipes.search_recipes)
    2) Enrichment nur für Kandidaten
    3) Re-Scoring nach Zutaten-Treffern
    """
...
def handle(message: str, context: dict | None = None, use_llm: bool = False) -> Dict[str, Any]:
    ...
    if intent == "plan":
        ...
        results = tool_plan(q, top_k=5, strict=strict, exclude=ents["exclude"])
